...
from .serializers import (
    ...
    RecurringInvoiceCreateUpdateSerializer,
...
class RecurringInvoiceViewSet(viewsets.ModelViewSet):
    """
    ViewSet for RecurringInvoice CRUD operations.
    """

    queryset = RecurringInvoice.objects.all()
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['client', 'status', 'frequency']
    search_fields = ['template_name']
    ordering_fields = ['created_at', 'next_run_date', 'amount']
    ordering = ['-created_at']

    def get_serializer_class(self):
        if self.action in ['create', 'update', 'partial_update']:
            return RecurringInvoiceCreateUpdateSerializer
        return RecurringInvoiceSerializer

    def get_queryset(self):
        queryset = super().get_queryset().select_related('client', 'contract', 'owner')
        if not self.request.user.is_staff:
            queryset = queryset.filter(
                Q(owner=self.request.user) | Q(client__owner=self.request.user)
            )
        return queryset
    ...
    def pause(self, request, pk=None):
        ...
        if recurring.status != RecurringInvoice.ACTIVE:
            return Response(
                {'detail': 'Only active recurring invoices can be paused.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        recurring.status = RecurringInvoice.PAUSED
    ...
    def resume(self, request, pk=None):
        ...
        recurring = self.get_object()
        if recurring.status != RecurringInvoice.PAUSED:
            return Response(
                {'detail': 'Only paused recurring invoices can be resumed.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        recurring.status = RecurringInvoice.ACTIVE
        recurring.save(update_fields=['status', 'updated_at'])
        serializer = self.get_serializer(recurring)
        return Response(serializer.data)
    ...
    def generate_now(self, request, pk=None):
        """Generate an invoice immediately from this recurring template."""
        recurring = self.get_object()
        try:
...
class LateFeePolicyViewSet(viewsets.ModelViewSet):
    """
    ViewSet for LateFeePolicy CRUD operations.
    """
    ...
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['fee_type', 'is_active', 'apply_frequency']
    search_fields = ['name']
    ordering = ['name']
...
class PaymentReminderViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['invoice', 'reminder_type', 'status']
    ...
    def send_now(self, request, pk=None):
        ...
        if reminder.status != PaymentReminder.SCHEDULED:
            return Response(
                {'detail': 'Only scheduled reminders can be sent.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        reminder.mark_sent()
    ...
    def cancel(self, request, pk=None):
        ...
        if reminder.status != PaymentReminder.SCHEDULED:
            return Response(
                {'detail': 'Only scheduled reminders can be cancelled.'},
                status=status.HTTP_400_BAD_REQUEST
            )
