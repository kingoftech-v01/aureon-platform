...
class PortalMessageViewSet(viewsets.ModelViewSet):
    """
    ViewSet for portal message management.
    """

    queryset = PortalMessage.objects.all()
    serializer_class = PortalMessageSerializer
    permission_classes = [IsAuthenticated]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = ['client', 'sender', 'is_from_client', 'is_read']
    search_fields = ['subject', 'content']
    ordering = ['-created_at']

    def get_queryset(self):
        """Filter queryset based on user permissions."""
        queryset = super().get_queryset().select_related('client', 'sender')

        if not self.request.user.is_staff:
            queryset = queryset.filter(
                Q(client__owner=self.request.user) |
                Q(sender=self.request.user) |
                Q(client__portal_user=self.request.user)
            )

        return queryset

    def perform_create(self, serializer):
        """Set sender to current user on create."""
        user = self.request.user
        is_from_client = hasattr(user, 'client_profile')
        serializer.save(sender=user, is_from_client=is_from_client)
    ...
    def unread_count(self, request):
        ...
        count = queryset.filter(is_read=False).exclude(sender=request.user).count()
